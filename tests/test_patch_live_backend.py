...
from scripts.patch_live_backend import BACKEND_MARKER, DUAL_SCOPE_MARKER, EVAL_SCOPE_MARKER, SEAL_SEMANTIC_MARKER, patch_backend
...
class BackendPatchTests(unittest.TestCase):
    def test_patch_is_idempotent_and_adds_contract(self):
        ...
        self.assertIn('"labeled_evaluation": labeled_evaluation', once)
        self.assertIn('SEAL_OCR_CSV', once)
        self.assertIn('rows[:limit]', once)
        self.assertIn('"dashboard_full_labeled_set"', once)
