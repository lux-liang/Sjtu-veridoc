...
class SealOcrHelperTests(unittest.TestCase):
    def test_polar_unwrap_shape(self):
        ...
        draw.ellipse((35, 35, 265, 265), outline=40, width=8)
        unwrapped = polar_unwrap(image, radial_samples=80, angular_samples=360)
        self.assertEqual(unwrapped.size, (360, 80))
    ...
    def test_partial_entity_match_handles_ocr_noise(self):
        ...
        self.assertEqual(partial_similarity("A", "上海示例科技有限公司"), 0.0)

    def test_normalization_keeps_chinese_and_alphanumeric(self):
        self.assertEqual(normalize_text(" A-上海（测试）123 "), "a上海测试123")

    def test_repeated_dense_center_candidate_is_logo(self):
        row = {
    ...
    def test_repeated_large_watermark_is_unknown_not_logo(self):
        row = {
    ...
    def test_position_semantics_are_business_scoped(self):
        ...
        self.assertEqual(
            position_assessment("contract", "seal", "0.5,0.72,0.7,0.90"),
            ("bottom", "expected_signature_zone"),
        )
        self.assertEqual(
            position_assessment("contract", "logo", "0.1,0.02,0.2,0.12"),
            ("top", "not_applicable"),
        )
