...
def candidate_semantics(candidate: dict, page_width: int, page_height: int) -> dict:
    ...
    center_y = ((y0 + y1) / 2.0) / max(page_height, 1)
    ...
    bottom_stamp = (
        center_y >= 0.55
        and 0.50 <= pixel_aspect <= 1.85
        and 0.04 <= area_ratio <= 0.075
        and ink_ratio <= 0.14
        and ring_density >= 0.08
        and angular_coverage >= 0.85
        and (
            (ring_uniformity >= 0.68 and center_ring_ratio <= 0.65)
            or (ring_uniformity >= 0.58 and center_ring_ratio >= 0.80)
        )
    )
    header_emblem = center_y < 0.22 and saturation >= 0.12 and area_ratio <= 0.03
    ...
    if dense_square:
        ...
        candidate_class = "unknown"
        class_reason = "header_emblem_pattern"
        ocr_recommended = 0
        ...
        ocr_recommended = 0
    elif outer_ring or color_stamp or monochrome_stamp or bottom_stamp:
        candidate_class = "seal"
        class_reason = "bottom_stamp_geometry" if bottom_stamp and not (outer_ring or color_stamp or monochrome_stamp) else "annular_stamp_geometry"
        ocr_recommended = 1
    else:
        candidate_class = "unknown"
